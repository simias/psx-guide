...
\lstdefinelanguage{assembly}%
  {
   morekeywords={nop,lui,move,addu,addiu,addi,j,jal,sll,%
                 sw,sh,sb,or,ori,%
                 mtc0,mtc1,mtc2,mtc3,%
                 bne,%
                 },%
...
The next instruction is \texttt{0x0bf00054} which is a jump
...
the target address \texttt{0xbfc00150}.
...
expensive (and why I always overwrite \texttt{regs[0]} in
\texttt{set\_reg} instead of checking if the register was 0).
...
Consider the following assembly\footnote{I'm assuming that
  ...
  behaviour you have to use ``\texttt{.set noreorder}'' with the GNU
  assembler.}:
...
\texttt{foo}. When the function is entered \$a0 will be equal to
\texttt{0x0f000000}.
...
of values in the \texttt{SYS\_CONTROL} registers that we chose to
ignore. We then stumbble upon a new instruction: \texttt{0x00000825}
...
right now the helper methods in the \texttt{Instruction} return
register indexes as \texttt{u32}. The same type as the values
...
number \texttt{s} it ORs the index \texttt{s} itself. It's meaningless
...
incompatible with \texttt{u32}:

\begin{lstlisting}
struct RegisterIndex(u32);
\end{lstlisting}

Note that this is not like a \texttt{typedef} in C or C++:
\texttt{typedef} just creates an alias which remains compatible
(i.e. interchangeable) with the original type. The equivalent in C
would be to wrap the \texttt{u32} in a \texttt{struct} or something like that.

Then we just have to update our helpers as well as the
\texttt{Cpu::reg} and \texttt{Cpu::set\_reg} methods to use a
\texttt{RegisterIndex} instead of a plain \texttt{u32}.

With this modification the compiler will reject the broken
\texttt{op\_or} implementation above:
...
The BIOS then wants to write \texttt{0x00000804} to
\texttt{0xfffe0130}. This address is used for cache control. Since we
...
The next unhandled instruction, \texttt{0x408c6000}, involves one of
...
Back to the \texttt{0x408c6000} instruction: the opcode (bits [31:26])
is equal to \texttt{0b010000} which means that it's an instruction for
the coprocessor 0. The generic format is \texttt{0b0100}\emph{nn} where
...
\texttt{Instruction::cop\_opcode} returns the same bit range as
\texttt{Instruction::s}, however it returns it as a plain \texttt{u32}
instead of a \texttt{RegisterIndex} (since it's not a register in this
case). You see that the current coprocessor opcode \texttt{0b00100} means
...
if we decode the instruction in full it gives\footnote{This is
  ...
  assembler syntax would be \mbox{\texttt{mtc0 \$12, \$12}} but it's a
...
\texttt{\$cop0\_12} is a very useful register: it's called the
...
At this point the \$12 register contains \texttt{0x00010000} so this
...
we'll just store the value of the SR in our \texttt{Cpu} struct and
move along:
...
We now encounter the instruction \texttt{0x154bfff7}. It encodes a
BNE or *branch if not equal* instruction. The difference between
jumps and branches is that branches are *conditional* and they use
*relative offsets*.
...
Like jumps, branches have a delay slot\footnote{It is called a
  \emph{branch} delay slot after all\ldots}. Fortunately our
...
Notice the \texttt{wrapping\_sub(4)} to compensate for our hardcoded
\texttt{pc.wrapping\_add(4)} in
\texttt{run\_next\_instruction}. Without it we'd branch one
instruction too far.

\subsection{ADDI instruction}

Before we even reach the target of the branch we stumble upon
unhandled instruction \texttt{0x214a0080}. This one is an ADDI which
...
to \texttt{i32} before using the \texttt{checked\_add} provided by
...
The cast to \texttt{i32} is important because something like
\texttt{0x4 + 0xffffffff} is an overflow in 32bit unsigned
...
would be the same (\texttt{0x00000003}) but since ADDI generates an
